...
session_start();
$result2 = mysqli_query($conn,"SELECT * FROM `postjob`");
    ...
    <table cellpadding="0" cellspacing="0" border="0">
      <thead>
        <tr>
          ...
          <th style="background-color: black;color:white;border:solid 2px white;text-align: center;">APPLY FOR JOB</th>
        </tr>
      </thead>
    </table>
  </div>
    ...
    <table cellpadding="0" cellspacing="0" border="0">
      <tbody>
        <tr>
          ...
          <td style="color:black;text-align: center;font-weight: bold;"><a href="applyforjob.php?username=<?php echo $_SESSION['username']; ?>&id=<?php echo $row3['id'];?>&universityname=<?php echo $row3['universityname'];?>&designation=<?php echo $row3['designation'];?>&subject=<?php echo $row3['subject'];?>&exp=<?php echo $row3['exp'];?>&salary=<?php echo $row3['salary'];?>"><button style="background-color: red;color: white;">APPLY</button></a></td>
        </tr>
      </tbody>
    </table>
  </div>
